...
const SearchFilter = ({
  ...
  const [filteredSoundcloudTracks, setFilteredSoundcloudTracks] =
    useState<Track[]>(soundcloudTracks);
  ...
  useEffect(() => {
    if (!searchQuery.trim()) {
      ...
      const filteredSpotify = spotifyTracks.filter(
        (track) =>
          track.title.toLowerCase().includes(query) ||
          track.artistInfo.some((artist) => artist.name.toLowerCase().includes(query))
      );

      // Filter SoundCloud tracks
      const filteredSoundcloud = soundcloudTracks.filter(
        (track) =>
          track.title.toLowerCase().includes(query) ||
          track.artistInfo.some((artist) => artist.name.toLowerCase().includes(query))
      );
              ...
              {activeTab !== "all" && (
                <span>
                  ...
                  {activeTab === "spotify"
                    ...
                    : filteredSoundcloudTracks.length}{" "}
                  in current tab)
                </span>
              )}
            </span>

...
const AddToQueueAction = ({track}: AddToQueueProp): JSX.Element => {
  ...
  const isInQueue = queue.some(
    (queuedTrack) => queuedTrack.id === track.id && queuedTrack.provider === track.provider
  );
  ...
  return (
    <div className="inline-flex items-center justify-center h-8 w-8 relative">
      <button
        ...
        disabled={isInQueue}
        className={`flex items-center justify-center w-6 h-6 transition-transform ${
          isInQueue ? "opacity-30 cursor-not-allowed" : "hover:scale-110"
        }`}
        aria-label={isInQueue ? "Already in queue" : "Add to queue"}
        title={isInQueue ? "Already in queue" : "Add to queue"}>
        <img className="w-full h-full" src={AddToQueueSVG} alt="Add to queue" />
      </button>

...
export const PlayerStateProvider = ({children}: ProviderProps) => {
  ...
  useEffect(() => {
    ...
    const handlePlay = () => {
      ...
      // Use the volume from settings consistently
      if (currentVolumeRef.current !== null) {
        // SoundCloud expects 0-100 range
  ...
  const playCurrentTrack = async (index: number | null = currentTrackIndex) => {
    ...
    if (currentTrack.provider === "spotify" && spotifyPlayerRef.current) {
      ...
      if (currentVolumeRef.current !== null && spotifyPlayerRef.current) {

        ...
        <div className="flex flex-wrap items-center justify-between">
          <div className="flex">
            <button
              ...
              className={`py-2 px-4 ${
                activeTab === "all" ? "border-b-2 border-accentPrimary text-white" : "text-gray-400"
              }`}>
              All Favorites
            </button>
            <button
          ...
          <div className="overflow-y-auto max-h-[calc(100vh-300px)]">
            ...
            {filteredSpotifyTracks.length === 0 && filteredSoundcloudTracks.length === 0 && (
              <div className="text-center py-10">
                <p className="text-gray-400">
                  No matching tracks found. Try adjusting your filter.
                </p>
              </div>
            )}
            ...
            {activeTab === "all" ? (
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                {/* SoundCloud column */}
                {sortedSoundcloudTracks.length > 0 && (
                  <div>
                    <h2 className="text-xl font-medium mb-3 flex items-center gap-2">
                    ...
                    <div className="space-y-2 bg-gray-800 bg-opacity-30 p-4 rounded-lg">
                      {sortedSoundcloudTracks.map((track) => (
              ...
              <ProviderFavoritesView tracks={sortedSpotifyTracks} provider="spotify" />
            ) : (
              <ProviderFavoritesView tracks={sortedSoundcloudTracks} provider="soundcloud" />
            )}
          </div>
        )}
      </div>
...
const EmptyFavoriteState = () => (
  <div className="flex flex-col items-center justify-center py-16 text-center">
    ...
    <Link
      to="/home"
      className="inline-block px-6 py-2 bg-accentPrimary text-white rounded-full text-center">
      Explore Music
    </Link>
  </div>
);
...
const ProviderFavoritesView = ({ tracks, provider }: { tracks: Track[]; provider: string }) => (
  <div className="space-y-2">
    {tracks.length > 0 ? (
      tracks.map((track) => <ItemCard key={track.id} item={track} provider={provider} />)
    ) : (
      <div className="text-center py-10">
        <p className="text-gray-400">
          No matching {provider === "spotify" ? "Spotify" : "SoundCloud"} favorites
        </p>
      </div>
    )}
  </div>
